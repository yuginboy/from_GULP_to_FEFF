...
def f_PM(x, n, J=2.5):
    # n = N/V
    return n*1e27*J*g_e*mu_Bohr*f_Brillouin(x, J)

#  fit to a global function
# def func(x, A, B, x0, sigma):
#     return A+B*np.tanh((x-x0)/sigma)

def f_diff_PM_for_2_T (B, n, J=2.5, T1=2, T2=5):
    # diff of 2 f_PM function for substrate FM component
    x1 = (g_e*J*mu_Bohr*B)/k_b/T1
    x2 = (g_e*J*mu_Bohr*B)/k_b/T2
    return f_PM(x1, n, J=J) - f_PM(x2, n, J=J)

def f_PM_with_T (B, n, J=2.5, T=50):
    # PM component by using a Brillouin function
    x = (g_e*J*mu_Bohr*B)/k_b/T
    return f_PM(x, n, J=J)

def f_SPM_with_T (B, n, J=2.5, T=50):
    # PM component by using a Langevin function
    x = (g_e*J*mu_Bohr*B)/k_b/T
    return n*1e27*J*g_e*mu_Bohr*f_Langevin(x)
...
if __name__ == '__main__':
    ...
    plt.plot(B,func(x,n=return_fit_param(x,y)),'r-', B,M,'.-')
    plt.draw()
    plt.show()
    popt, pcov = curve_fit(func, x, y)
    print('finished')